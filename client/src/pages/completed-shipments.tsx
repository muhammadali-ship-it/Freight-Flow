...
export default function CompletedShipments() {
    ...
    const { data: shipmentsData, isLoading } = useQuery<ShipmentsResponse>({
        queryKey: [
            "/api/shipments",
            {
                ...
                userId: user?.id,
                userRole: user?.role,
                isCompleted: true, // Fetch ONLY completed shipments
            },
        ],
        queryFn: async () => {
    ...
    useEffect(() => {
        setPage(1);
    }, [searchQuery, carrierFilter, originPortFilter, destinationPortFilter, dateFrom, dateTo]);
                                    ...
                                    shipments.map((shipment) => (
                                        <TableRow key={shipment.id} data-testid={`row-shipment-${shipment.id}`}>
                                            ...
                                            <TableCell>
                                                ...
                                                >
                                                    {getStatusLabel(shipment.status || "completed")}
                                                </Badge>
                                            </TableCell>
