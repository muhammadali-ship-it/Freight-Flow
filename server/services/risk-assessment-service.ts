...
export class RiskAssessmentService {
  ...
  async assessContainerRisk(container: Container): Promise<RiskAssessment> {
    ...
    // Logic: If "Vessel Arrival" at destination has NOT occurred and ETA is in the past -> Delayed.
    // If "Vessel Arrival" at destination HAS occurred -> Not Delayed (even if ETA was in the past).

    const milestones = await storage.getMilestones(container.shipmentId);

    // Find destination port from container or shipment
    const destination = container.destination || "";

    const vesselArrivalAtDest = milestones.find(m =>
      (m.eventType === "Vessel Arrival" || m.eventType === "Vessel Arrival AIS") &&
      m.timestampActual &&
      (
        !destination || // If no destination known, assume it's the right one (fallback)
        (m.location && m.location.toLowerCase().includes(destination.toLowerCase())) ||
        (destination.toLowerCase().includes(m.location?.toLowerCase() || ""))
      )
    );
    ...
    if (container.eta && !["arrived", "unloaded", "gate-out", "delivered"].includes(container.status)) {
      ...
      // If vessel has arrived at destination, we do NOT consider it delayed for ETA purposes
      if (!vesselArrivalAtDest) {
